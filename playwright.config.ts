 ...
 *
 * `webServer`: Playwright tự build + start Next rồi tự tắt. Dùng `next start` (production
 * build) chứ không phải `next dev`, vì CSP ở dev có nới thêm `'unsafe-eval'`/`'unsafe-inline'`
 * cho React Refresh — test trên dev sẽ đo nhầm một CSP không tồn tại ở production.
 */
const PORT = 3123;
